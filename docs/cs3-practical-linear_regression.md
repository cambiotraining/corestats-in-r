...
Let's see if the `Murder` variable can be used to predict the `Life.Exp` variable. Let's plot it first of all.
...
Now let's investigate the relationship between the proportion of High School Graduates a state has (`HSGrad`) and the mean number of days below freezing (`Frost`) within each state.
...
* Normality is pretty good from the Normal Q-Q plot
* Homogeneity of variance isn't very good and there appears to be a noticeable drop in variance as we go from left to right (from consideration of the Scale-Location plot)
* There don't appear to be any influential points (by looking at the residuals vs leverage graph)
